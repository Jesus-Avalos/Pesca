            ...
            <v-card>
                <v-toolbar dark color="primary">
                    ...
                    <v-toolbar-items>
                        ...
                        <v-btn text type="submit" :form='"equipoForm"+datosO.id'>Registrar</v-btn>
                    </v-toolbar-items>
                </v-toolbar>

                <div class="m-5">
                    <div class="alert bg-success" :id='"alertSuccess"+datosO.id' style="display: none">
                        Registrado correctamente
                    </div>
                    <v-dialog v-model="dialogGolon" persistent width="500">
                        <v-card>
                           <v-card-title class="headline">Registro de golón</v-card-title>
                           <form @submit.prevent="saveGolon" :id='"golonEquipo"+datosO.id'>
                               <div align="center" class="form-group">
                                    <label>Integrante del equipo</label>
                                    <v-select :items="personas" item-text="name" item-value="id" v-model="persona_id" label="Solo field" solo class="w-50"></v-select>
                                </div>
                                <div align="center" class="form-group">
                                    <label>Peso de la especie</label>
                                    <input type="text" class="form-control w-50 text-center" v-model="golon" required>
                                </div>
                                <v-card-actions>
                                <v-spacer></v-spacer>
                                <v-btn color="blue-grey darken-2" class="white--text" rounded @click="dialogGolon = false">Cancelar</v-btn>
                                <v-btn color="green darken-1" :form='"golonEquipo"+datosO.id' class="white--text" rounded type="submit">Registrar</v-btn>
                                </v-card-actions>
                           </form>
                        </v-card>
                    </v-dialog>
                    <form :id='"equipoForm"+datosO.id' @submit.prevent="saveData">
                        ...
                        <div class="from-group float-right mt-4">
                            <label for="TotaEquipo">Total de equipo: </label>
...
export default {
    ...
    data() {
        return {
            ...
            personas: (JSON.parse(this.datos)).personas,
            persona_id: 0,
            golon: 0,
            bloques: [],
            dialog: false,
    ...
    methods: {
        saveData() {
            ...
            const url = 'http://pesca.test/api/equipo/' + this.datosO.pivot.id;
